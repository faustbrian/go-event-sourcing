...
func TestAtomicityFailureAfterEveryWriteCannotBeCommitted(t *testing.T) {
	...
	for _, fault := range []string{
		...
		t.Run(fault, func(t *testing.T) {
			...
			if _, err := tx.Exec(
				ctx,
				"SELECT set_config('eventoutbox.fail_point', $1, true)",
				fault,
			); err != nil {
				t.Fatal(err)
			}
...
func TestSavepointCleanupFailureDoesNotRollBackCallerTransactionOrCommitSplitState(
	...
	codec, err := eventoutbox.NewEnvelopeCodec(
		eventoutbox.FixedTopic("atomicity-events"),
		codecLimits,
	)
	if err != nil {
		t.Fatal(err)
	...
	if !errors.Is(stageErr, eventoutbox.ErrEnvelopeEncoding) ||
...
LANGUAGE plpgsql AS $$
DECLARE
    configured text := current_setting('eventoutbox.fail_point', true);
...
func newAtomicityStager(t *testing.T, tx pgx.Tx) *eventoutbox.Stager {
	t.Helper()
...
func atomicityDependencies(
	t *testing.T,
) (*outboxpostgres.Writer, *eventoutbox.EnvelopeCodec) {
	t.Helper()
