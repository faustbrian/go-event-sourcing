...
func TestCleanupKafkaContainerRegistersTermination(t *testing.T) {
	t.Parallel()

	container := &kafkaContainerCleanupStub{containerID: "registered-container"}
	t.Run("fixture", func(t *testing.T) {
		cleanupKafkaContainer(t, container)
	})
	if container.terminations != 1 {
		t.Fatalf("container terminations = %d, want 1", container.terminations)
	}
}

func TestCleanupKafkaContainerAcceptsNil(t *testing.T) {
	t.Parallel()

	cleanupKafkaContainer(t, nil)
}
