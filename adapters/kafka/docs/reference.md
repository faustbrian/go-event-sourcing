...
```go
dispatcher, err := kafka.NewDispatcher(producer, codec)
if err != nil {
	return err
}
...
handler, err := kafka.NewRecordHandler(codec, projectionConsumer)
if err != nil {
	return err
}
...
```go
policy, err := kafka.NewDeadLetterPolicy(
	producer,
	kafka.DeadLetterPolicyConfig{
		Topic:  "accounts.events.dead-letter.v1",
		Limits: kafka.DefaultRecordLimits(),
	},
)
if err != nil {
	return err
}
handler, err := kafka.NewRecordHandler(
	codec,
	projectionConsumer,
	kafka.WithFailurePolicy(policy),
)
```
...
From this module directory, run `make check` for the focused contract. From the
repository root, run `make check` for the complete repository contract. The
focused command is a development loop and is not release evidence. With Docker available,
