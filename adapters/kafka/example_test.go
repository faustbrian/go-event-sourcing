...
func ExampleNewDispatcher() {
	...
	codec, err := eventkafka.NewRecordCodec(eventkafka.RecordCodecConfig{
		Resolver:      eventkafka.FixedTopic("accounts.events.v1"),
	...
	dispatcher, err := eventkafka.NewDispatcher(producer, codec)
	if err != nil {
...
func ExampleNewRecordHandler() {
	...
	handler, err := eventkafka.NewRecordHandler(
		codec,
		eventkafka.DeliveryConsumerFunc(persistProjection),
	)
	if err != nil {
		log.Fatal(err)
	}
