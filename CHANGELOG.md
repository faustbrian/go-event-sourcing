...
- Link every releasable module to the versioned Golib ecosystem design
  language and package-selection index.
...
- Add a checksum-pinned first-party JSON and upcast compatibility corpus with
  provenance and focused exact-byte assertions.
- Compare public aggregate reconstruction across the pinned Go event-sourcing
  ...
  checks, explicit accounting for implementation-required work, and published
  raw and statistically summarized evidence.
...
- Add runnable synchronous-dispatch, snapshot-restoration, and aggregate
  scenario examples for the remaining core adoption workflows.
- Add a runnable workflow catalog that binds core, PostgreSQL, Kafka, queue,
  outbox, and telemetry documentation to executable examples and integration
  tests.
