...
func TestEncodedEventValidationAndOwnership(t *testing.T) {
	name, err := eventsourcing.NewEventName("account.opened_2")
	if err != nil || name.String() != "account.opened_2" || name.IsZero() {
		t.Fatalf("NewEventName() = %#v, %v", name, err)
	}
	if !(eventsourcing.EventName{}).IsZero() {
		t.Fatal("zero EventName did not report zero")
	}
	for _, value := range []string{
		"",
		"Account.Opened",
		"account..opened",
		strings.Repeat("a", eventsourcing.MaxEventNameBytes+1),
	} {
		if _, nameErr := eventsourcing.NewEventName(value); !errors.Is(
			nameErr,
			eventsourcing.ErrInvalidArgument,
		) {
			t.Fatalf("NewEventName(%q) error = %v", value, nameErr)
		}
	}
