...
// RestoreSnapshotVersion establishes the committed version represented by
// already decoded aggregate snapshot state.
func (lifecycle *Lifecycle) RestoreSnapshotVersion(version uint64) error {
	if lifecycle.poisoned {
		return ErrLifecyclePoisoned
	}
	if lifecycle.transition ||
	...
	if version == 0 {
		return invalid("snapshot_version", "must be greater than zero")
	}

	lifecycle.committed = version
	lifecycle.generation++

	return nil
}

// ReconstituteNext applies one stored stream version without retaining earlier
// history. An empty logical event slice represents an explicitly dropped
// stored event and still advances the committed version.
func (lifecycle *Lifecycle) ReconstituteNext(
	sourceVersion uint64,
	history []HistoricalEvent,
	apply func(DecodedEvent) error,
) error {
	...
	if sourceVersion == 0 || sourceVersion != lifecycle.committed+1 {
		lifecycle.poisoned = true

		return fmt.Errorf("%w: invalid incremental source version", ErrCorruptHistory)
	}
	if err := validateHistory(lifecycle.committed, history); err != nil {
		lifecycle.poisoned = true

		return err
	}
	if len(history) != 0 &&
		history[len(history)-1].sourceVersion != sourceVersion {
		lifecycle.poisoned = true

		return fmt.Errorf("%w: incremental history crossed source versions", ErrCorruptHistory)
	}
	...
	lifecycle.committed = sourceVersion
	lifecycle.generation++
...
// MarkPersistenceUnknown poisons the exact current change set so callers must
// reconcile its message IDs with durable storage before taking further action.
func (lifecycle *Lifecycle) MarkPersistenceUnknown(changes ChangeSet) error {
	if lifecycle.poisoned {
		return ErrLifecyclePoisoned
	}
	if lifecycle.transition {
