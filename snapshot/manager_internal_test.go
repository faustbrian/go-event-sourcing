...
func TestNewManagerRejectsInvalidConfiguration(t *testing.T) {
	...
	tests := map[string]func(*ManagerConfig[string, *internalAggregate]){
		...
		"migrations": func(config *ManagerConfig[string, *internalAggregate]) {
			config.Migrations = &MigrationChain{}
		},
...
func TestManagerClassifiesSnapshotMigrationFailures(t *testing.T) {
	t.Parallel()
	...
	tests := map[string]struct {
		...
		"missing path": {
			migration: mustStateMigration(t, 2, 3, unchangedMigration),
			want:      eventsourcing.ErrSnapshotIncompatible,
			fallback:  true,
		},
		"corrupt state": {
			migration: mustStateMigration(
				t,
				1,
				2,
				func([]byte, map[string]string) ([]byte, map[string]string, error) {
					return nil, nil, eventsourcing.ErrSnapshotCorrupt
				},
			),
	...
	for name, testCase := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			chain, err := NewMigrationChain(testCase.migration)
			if err != nil {
				t.Fatal(err)
			}
			config.Migrations = chain
			...
			if testCase.fallback {
				if err != nil || aggregate == nil ||
					...
					!errors.Is(info.FallbackReason(), testCase.want) {
					t.Fatalf("Load() = %#v, %#v, %v", aggregate, info, err)
				}

				return
			}
			if aggregate != nil || info.Source() != LoadUnknown ||
...
func TestThresholdPolicyAndConditionalRefreshRejectInvalidState(t *testing.T) {
	t.Parallel()

	if policy, err := NewThresholdPolicy(0); policy.interval != 0 ||
		!errors.Is(err, eventsourcing.ErrInvalidArgument) {
		t.Fatalf("NewThresholdPolicy(0) = %#v, %v", policy, err)
	}
	policy, err := NewThresholdPolicy(2)
	if err != nil {
		t.Fatal(err)
	}
	manager := internalManager(t)
	...
	checks := []struct {
		name    string
		manager *Manager[string, *internalAggregate]
		ctx     context.Context
		id      string
		value   *internalAggregate
		version uint64
		policy  ThresholdPolicy
		want    error
	}{
		{"nil manager", nilManager, context.Background(), "id", aggregate, 1, policy, eventsourcing.ErrInvalidArgument},
		{"nil context", manager, nilContext, "id", aggregate, 1, policy, eventsourcing.ErrInvalidArgument},
		{"zero policy", manager, context.Background(), "id", aggregate, 1, ThresholdPolicy{}, eventsourcing.ErrInvalidArgument},
		{"identifier", manager, context.Background(), "other", aggregate, 1, policy, eventsourcing.ErrInvalidArgument},
		{"future snapshot", manager, context.Background(), "id", aggregate, 4, policy, eventsourcing.ErrInvalidLifecycleState},
	}
	for _, check := range checks {
		...
		t.Run(check.name, func(t *testing.T) {
			...
			created, refreshed, refreshErr := check.manager.RefreshIfDue(
				check.ctx,
				check.id,
				check.value,
				check.version,
			...
			if !created.IsZero() || refreshed || !errors.Is(refreshErr, check.want) {
				t.Fatalf("RefreshIfDue() = %#v, %t, %v", created, refreshed, refreshErr)
			}
		})
	}
	...
	encodeManager := managerFromInternalConfig(t, config)
	if _, refreshed, err := encodeManager.RefreshIfDue(
		context.Background(), "id", aggregate, 1, policy,
	); refreshed || !errors.Is(err, errManagerTest) {
		t.Fatalf("RefreshIfDue(encode failure) = %t, %v", refreshed, err)
	}

	config = internalConfig()
	config.Lifecycle = func(*internalAggregate) *eventsourcing.Lifecycle { return nil }
	nilLifecycle := managerFromInternalConfig(t, config)
	if _, refreshed, err := nilLifecycle.RefreshIfDue(
		context.Background(), "id", aggregate, 1, policy,
	); refreshed || !errors.Is(err, eventsourcing.ErrInvalidArgument) {
		t.Fatalf("RefreshIfDue(nil lifecycle) = %t, %v", refreshed, err)
	}

	poisoned := &internalAggregate{id: "id"}
	event, err := eventsourcing.NewDecodedEvent(eventsourcing.DecodedEventInput{
		Name: "changed", Version: 1, Value: "value",
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, refreshed, err := manager.RefreshIfDue(
		context.Background(), "id", poisoned, 0, policy,
	); refreshed || !errors.Is(err, eventsourcing.ErrLifecyclePoisoned) {
		t.Fatalf("RefreshIfDue(poisoned) = %t, %v", refreshed, err)
	}
	...
	if _, refreshed, err := manager.RefreshIfDue(
		context.Background(), "id", pending, 0, policy,
	); refreshed || !errors.Is(err, eventsourcing.ErrInvalidLifecycleState) {
		t.Fatalf("RefreshIfDue(pending) = %t, %v", refreshed, err)
	}

	config = internalConfig()
	config.Store = internalStore{
		load: func(context.Context, eventsourcing.StreamID) (eventsourcing.Snapshot, error) {
			return eventsourcing.Snapshot{}, eventsourcing.ErrSnapshotNotFound
		},
		save: func(context.Context, eventsourcing.Snapshot) error { return errManagerTest },
	}
	failingManager := managerFromInternalConfig(t, config)
	if _, refreshed, err := failingManager.RefreshIfDue(
		context.Background(), "id", aggregate, 1, policy,
	); refreshed || !errors.Is(err, errManagerTest) {
		t.Fatalf("RefreshIfDue(save failure) = %t, %v", refreshed, err)
	}
}
